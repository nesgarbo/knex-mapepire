...
class DB2Client extends knex.Client {
    ...
    async _query(
        pool: MapepirePool,
        obj: QueryObject & { sql: string; bindings?: BindingValue[] }
    ) {
    ...
    private async executeStatementQuery(
        ...
        try {
            ...
            const hasReturning =
                Array.isArray(obj.returning) && obj.returning.length > 0;
    ...
    async _stream(
        ...
        throw new Error(
            "Streaming no soportado actualmente con Mapepire. Usa paginación."
        );
    }
...
function makeFeathersPoolView(mp: MapepirePool, cfg: any) {
    ...
    const numUsed = () =>
        typeof (mp as any).getActiveJobCount === "function"
    ...
    // --- helper para un “acquire” compatible con ambos estilos ---
    const createAcquireReturn = () => {
        const promise = Promise.resolve(mp as any); // devolvemos el “conn” que espera tu client: el propio MapepirePool
        // Devolvemos objeto con .promise (estilo Tarn) y también thenable (por si hacen .then directamente)
        const thenable: any = {
            promise,
    ...
    return {
        ...
        release(_conn?: any) {
            // no-op: no “resource” por conexión; tú usas el pool directamente
            return;
        },

        destroy(_conn?: any) {
            // Por si algún sitio “destruye” el recurso
            mp.end();
        },
    };
}
